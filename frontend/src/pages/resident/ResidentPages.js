...
import { Receipt, CreditCard, Clock, CheckCircle, XCircle, IndianRupee, Wallet } from 'lucide-react';
          ...
          <div className="flex items-start justify-between">
            <div>
              ...
              <p className="text-3xl font-bold text-info mt-2">
                {bills.filter(b => b.status === 'verified').length}
              </p>
...
const MyBills = () => {
  ...
  const getStatusBadge = (bill) => {
    switch (bill.status) {
      ...
      case 'paid':
        return <span className="badge-info">Awaiting Verification</span>;
      case 'pending':
    ...
    <div data-testid="my-bills-page" className="space-y-6">
                ...
                bills.map((bill) => (
                  <tr key={bill.id} className="table-row">
                    ...
                    <td className="p-4 text-text-secondary text-sm">
                      {bill.payment_mode ? (
                        <span>{bill.payment_mode.toUpperCase()} - {bill.payment_ref}</span>
                      ) : (
                        '-'
                      )}
                    </td>
      ...
      <Dialog open={showPayModal} onOpenChange={setShowPayModal}>
        <DialogContent className="bg-bg-surface border-border-color">
          <DialogHeader>
            <DialogTitle className="text-text-primary">Submit Payment</DialogTitle>
          </DialogHeader>
...
const WalletPage = () => {
  ...
  useEffect(() => {
    const fetchWallet = async () => {
      try {
        const res = await maintenanceAPI.getWallet();
        setWallet(res.data);
      } catch (e) {
    ...
    <div data-testid="wallet-page" className="space-y-6">
      ...
      <div className="card p-8 max-w-md">
        ...
        <p className="text-text-secondary text-sm">
          ₹1 = 1 point. Points are credited when your payment is verified.
        </p>
      </div>
