...
import { 
  Building2, LayoutDashboard, CreditCard, FileText, ClipboardCheck,
  BarChart3, Bell, MessageSquare, Wallet, LogOut, Menu, X, ChevronDown
} from 'lucide-react';
...
const SubAdminLayout = ({ children }) => {
  ...
  const navItems = [
    { path: '/subadmin', icon: LayoutDashboard, label: 'Dashboard' },
    { path: '/subadmin/payments', icon: CreditCard, label: 'Verify Payments' },
    { path: '/subadmin/plans', icon: ClipboardCheck, label: 'Plan Approvals' },
    { path: '/subadmin/expenses', icon: FileText, label: 'Expense Approvals' },
    { path: '/subadmin/wallet', icon: Wallet, label: 'Wallet' },
    { path: '/subadmin/reports', icon: BarChart3, label: 'Reports' },
      ...
      <div className="lg:hidden fixed top-0 left-0 right-0 z-50 bg-bg-surface border-b border-border-color h-16 flex items-center justify-between px-4">
        <div className="flex items-center gap-3">
          ...
          <div className="flex items-center gap-2">
            ...
            <span className="font-semibold text-text-primary">Sub-Admin</span>
          </div>
        </div>
      </div>
