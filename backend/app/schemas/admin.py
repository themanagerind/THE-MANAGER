...
class AdminSignupIn(BaseModel):
    ...
    Owner or Tenant — same as ResidentSignupIn, a Tenant here does NOT
    require the property to already have an active Owner (Section 12
    invariant); the account stays PENDING until Platform Owner review.
    There is no "describe a brand-new unit" option here — the Platform
    Owner is expected to have mapped the society's Wings/Rows/flats
    before an Admin signs up against it. Nothing is created until
    Platform Owner approval activates the account, same as the ADMIN
    role itself."""
